$(function() {
    ...
    $('.movie-button').on('click', function(){
        var my_api_key = 'ef3c09c7854f3a776622d76007993eac';
        var title = $('.movie').val();
        if (title=="") {
...
function get_movie_details(movie_id, my_api_key, movie_title, movie_title_org) {
    $.ajax({
        type:'GET',
        url: 'https://api.themoviedb.org/3/movie/'+movie_id+'?api_key='+my_api_key+'&language=en-US',
...
function show_details(movie_details, movie_title, my_api_key, movie_id, movie_title_org){
    ...
    // calling 'get_recommnedations' to get the recommended movies for the given id from the TMDB API
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if ($('.movie-content')) {
                $('.movie-content').after('<div class="gottotop"><i title="Go to Top" class="fa fa-arrow-up"></i></div>');
            }
            $(window).scrollTop(0);
        }
    });
